...
실행: cd backend && python scripts/migrate_exam_sets_pk.py
...
def main():
    ...
    updates = []
    for i, row in enumerate(rows, start=2):  # 1행=헤더, 데이터는 2행부터
        d = repo._row_to_dict(row)
        if d.get("exam_id"):
            continue
        exam_set_id = d.get("exam_set_id")
        if not exam_set_id:
            continue
        updates.append({"range": f"{SHEET_TAB}!I{i}", "values": [[exam_set_id]]})
