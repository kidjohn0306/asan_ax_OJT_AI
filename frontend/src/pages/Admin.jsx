...
import { PlannedQuestionBank, PlannedQuestionGeneration, PlannedQuestionReview } from '../admin/pages/questions/PlannedQuestionPages'
...
// path를 즐겨찾기 식별자로 쓴다 — view는 여러 메뉴가 공유할 수 있어 식별자로 부적합하다.
...
export function ExamSheet({ toast, onNavigate, sourceExamId = null, onSaved }) {
  ...
  const [examCategory, setExamCategory] = useState('exam_study')
  ...
  useEffect(() => {
    ...
    apiFetch('GET', `/api/admin/exam-sets/${encodeURIComponent(sourceExamId)}/questions`)
      .then(data => {
        ...
        setExamCategory(source.exam_category || 'exam_study')
  ...
  async function handleSave() {
    ...
    try {
      ...
      const body = { name: examName.trim(), team_code: team, question_ids, exam_category: examCategory }
      if (sourceExamId) {
            ...
            <div style={{ display:'flex', gap:6 }}>
              {[['exam_study', '평가'], ['exam_test', '연습문제']].map(([value, label]) => (
                <button
                  key={value}
                  type="button"
                  onClick={() => setExamCategory(value)}
                  style={{ flex:1, height:44, border:`1.5px solid ${examCategory === value ? 'var(--accent)' : 'var(--border)'}`, background: examCategory === value ? 'var(--accent-light)' : 'white', color: examCategory === value ? 'var(--accent-dark)' : 'var(--text-muted)', borderRadius:8, fontFamily:'var(--font)', fontSize:13, fontWeight: examCategory === value ? 700 : 400, cursor:'pointer' }}
                >{label}</button>
              ))}
            </div>
          </div>
