...
describe('routed latest-main exam management', () => {
  ...
  it('keeps 60/70 defaults and navigates to the created exam URL with the existing payload', async () => {
    ...
    renderAdmin()
    await screen.findByText('시험 일정 운영')
    await userEvent.click(screen.getByRole('button', { name:'새 시험 만들기' }))
    await userEvent.click(screen.getByRole('button', { name:/시험지 선택 레이어 열기/ }))
    await userEvent.click(screen.getByRole('option', { name:/기초 시험지/ }))
    await userEvent.click(screen.getByRole('button', { name:'이 시험지 선택' }))
    expect(screen.getByText(/잠금 · 1팀/)).toBeInTheDocument()
    ...
    expect(screen.getByLabelText('시험 일시').value).toMatch(/T09:00$/)
    await userEvent.click(screen.getByRole('button', { name:'다음' }))
    await userEvent.click(screen.getByRole('button', { name:'시험 생성' }))
